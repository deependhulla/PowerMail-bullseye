...
MailScanner::Log::InfoLog('idheader1'); 
...
sub AppendSignCleanEntity {
  ...
  if ($MimeType =~ /text\/html/i) {
##################################
##$signature = "DVDX".$this->ReadVirusWarning('inlinehtmlsig');
my $dvdtail="";
my $dvdtailx="";
my $dvdmsgid="";
my $dvddate="";
...
my $dvdclick="";
foreach (@{$this->{headers}}) {
$dvdmsgid=$1 if /^Message-ID: (.*)/i;
$dvddate=Date::Parse::str2time($1) if /^Date: (.*)/i;
}
$dvdmsgid=~ s/ //eg;
$dvdtail=$dvddate."~~".$this->{'id'}."~~".$dvdmsgid."~~";

$dvdclick=$dvdurl."".$dvdtail."~~view.png";
...
###$signature = "DVDX--".$dvdtail."--".$this->{id}."--ZZZ".$this->ReadVirusWarning('inlinehtmlsig');
    foreach $line (@body) {
...
MailScanner::Log::InfoLog('idheader2'); 
...
my $idheaderxx='X-MailScanner-QID:';
    $global::MS->{mta}->ReplaceHeader($this, $idheaderxx, $this->{id});
MailScanner::Log::InfoLog('idheader3'); 
